...
def make_blastdb(longest_orfs):
	blastdb_log = subprocess.run(["makeblastdb","-in",longest_orfs,\
...
def blastp(blastdbName):
	opsinCandidates_log = subprocess.run(["blastp","-db", blastdbName,\
...
def mafft(opsinsFile):
	alignedOpsins = subprocess.check_output(["mafft","--add",opsinsFile,\
...
def raxml(opsinsPhylipFile):
	opsinTree = subprocess.run(["raxmlHPC-PTHREADS-SSE3","-f", "v",\
...
if len(sys.argv)<2:
	...
	sys.stderr.write("Current fasta file '{}'. Please include other inputs.\n".format(fastaName))


else:   
	newLongORFsFile= sys.argv[1] #short headers are recommended for the starting fastafile; e.g ">comp0_c0_seq1"
...
## Rooting the tree, then detaching the subtree with opsins only (no other GPCR)
	...
	opsinAnc = tree.get_common_ancestor("Patiria_miniata_ops5_chaopsin", "Cavia_porcellus_H0W479_RGR_retinochrome")
