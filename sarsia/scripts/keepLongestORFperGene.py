...
# 					duplicates += 1 #uncomment for testing
	...
	with open(longestORFperGeneFasta,"w") as outfile2:
		...
		short_seq_iterator = (record for record in input_seq_iterator for item in querylist if item.strip('\n') == record.id)
		SeqIO.write(short_seq_iterator,outfile2,"fasta")


if __name__ == '__main__':

	parser = argparse.ArgumentParser(description='Read FASTA files with open reading frames(ORFs) and nucleotides, and keep the longest ORF per gene')

	parser.add_argument('-p', metavar = 'FASTA_file', type = str, required = True,
					   help = "FASTA file with ORFs, peptides")
	parser.add_argument('-t', metavar = 'FASTA_file', type = str, required = True,
	...
	parser.add_argument('-identifier', metavar = 'identifier_type', type = str, choices=['type_1', 'type_2', 'type_3', 'type_4'], required = False,
