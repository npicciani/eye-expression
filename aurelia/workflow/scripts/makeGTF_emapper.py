...
python = "/gpfs/ysm/project/dunn/nnp9/conda_envs/emapper/bin/python" #path to python 3.7
...
for gene in geneID_proteinID_dict:
    ...
    if len(proteins) > 1:
        geneID_proteinID_dict[gene] = sorted(proteins, key=protein_length_dict.get, reverse =  True)
...
for gene in gene_goterm_dict:
    ...
    if len(goterms) >  0:
        for goterm in goterms:
            try:
            	goterm_outlist.append(gene + '\t' + goterm +  '\t' + gonames_dict[goterm])
            except KeyError:
            	pass
...
for transcript in transcript_length_dict:
    ...
    gtf_outlist.append(transcript + '\ttranscriptome\texon\t1\t' + str(transcript_length_dict[transcript]) + '\t.\t-\t.\tgene_id "' + gene + '"; transcript_id "' + transcript + '"; gene_name "' + gene_shortname_dict[gene] + '";')
